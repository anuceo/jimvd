...
fn main() -> Result<()> {
    let config_path = std::env::args().nth(1)
        .unwrap_or_else(|| "config/many_to_many.json".to_string());
    let config_str = fs::read_to_string(&config_path)?;
    let config: WorkloadConfig = serde_json::from_str(&config_str)?;

    println!("=== Many-to-Many Benchmark: {} ===", config.workload_name);
    println!("{}\n", config.description);
    ...
    // ---- Build query pool from raw JSON ----
    ...
    for q in raw_pool {
        ...
        for _ in 0..weight {
            match qtype {
                "eq" => {
                    let table = q["table"].as_str().unwrap_or("orders").to_string();
    ...
    for op in 0..total {
        ...
        if op >= warmup && op == next_snapshot {
            ...
            let o_count: usize = m_graph.tables.values()
                .map(|t| t.factors.values().filter(|f| !f.is_structural).count()).sum();
            println!(
                "[op {:>5}]  queries={:>5}  util={:>5.1}%  uaf={:.2}  fallbacks={:>4}  S/O={}/{}",
                op,
                metrics.total_queries.load(Ordering::Relaxed),
                metrics.factor_utilization() * 100.0,
                metrics.uaf(),
                metrics.join_fallbacks.load(Ordering::Relaxed),
                s_count, o_count,
            );
